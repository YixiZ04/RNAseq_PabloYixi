...
# Uso: ./01_qc_raw_reads.sh [-i <input_dir>] [-o <output_dir>] [-e <nombre_entorno>] [-h] [-v]
...
if [[ "$#" -eq 0 ]]; then
  print_error "Error: Requiere argumentos." >&2
  display_usage 
  exit 1
fi
...
exec > >(tee "${output_dir}"/logs/01_qc_raw_reads.out)
exec 2> >(tee "${output_dir}"/logs/01_qc_raw_reads.err)
...
# 9. Ejecutar MultiQC sobre los resultados de FastQC
